...
class Api {
  ...
  get_api_path(object, template) {
    const arrayOfKey = template.match(/(\w+)/g);
    if(arrayOfKey.includes("name")){
        let [api, items, id, name] = arrayOfKey
        name = object[name].split(" ")
        name.splice(1,0,`%${object[id]}`)
        let newName = name.join("")
        return `/${api}/${items}/${object[id]}/${newName}`
    }

    const [api, items, id, roleOrSalary] = arrayOfKey  
    return `/${api}/${items}/${object[id]}/${object[roleOrSalary]}`
  }
}
